...
**Step 0: Setup prometheus configuration**
There are 5 sources:
...
```
Put the IP in prometheus's [values.yaml](deployments/metrics/prometheus/values.yaml) at `serverFiles.prometheus.yaml.scrape_configs`:
```yaml
- job_name: "lens-app"
  scrape_interval: 5s
  static_configs:
