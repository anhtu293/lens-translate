...
```
- Get IP of **jaeger collector** SVC and put it in `model-serving`'s jaeger agent [values.yaml](deployments/model-serving/jaeger/values.yaml) at `agent.extraEnv.value`. For example: IP of **jaeger collector** is `34.172.165.133`:
```yaml
extraEnv:
